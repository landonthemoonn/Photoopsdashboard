...
import { Monitor, Search, RefreshCw, Circle, ExternalLink, WifiOff, Settings, Loader } from 'lucide-react';
...
export function Inventory() {
  ...
  const filtered = devices.filter(d => {
    ...
    const matchesSearch = d.name.toLowerCase().includes(q) || d.location.toLowerCase().includes(q) || d.category.toLowerCase().includes(q);
  ...
  return (
    <div>
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mb-6 flex items-end justify-between">
        ...
        <a href={JAMF_URL} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm" style={{ background: 'rgba(224,112,96,0.1)', border: '1px solid rgba(224,112,96,0.2)', color: 'var(--neon-blue)' }}>
      ...
      <div className="grid grid-cols-4 gap-3 mb-5">
        {[
          { label: 'Total Devices', value: live ? devices.length : '—', accent: '#E07060' },
          { label: 'Online', value: live ? devices.filter(d => d.status === 'online').length : '—', accent: '#8FBF8A' },
          { label: 'Offline', value: live ? devices.filter(d => d.status === 'offline').length : '—', accent: '#D86040' },
          { label: 'Needs Update', value: live ? devices.filter(d => d.updateStatus === 'needs-update').length : '—', accent: '#E09040' },
        ].map((s, i) => (
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="p-4 mb-4 flex items-center gap-3" style={glassCard}>
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Search by name, location, category..." value={search} onChange={e => setSearch(e.target.value)}
        ...
        <div className="flex gap-1 p-1 rounded-lg" style={{ background: 'rgba(255,255,255,0.04)' }}>
          {(['all', 'online', 'offline', 'needs-update'] as Filter[]).map(f => (
            <button key={f} onClick={() => setFilter(f)} className="px-3 py-1.5 rounded-md text-xs font-medium capitalize transition-all duration-200"
              style={{ background: filter === f ? 'rgba(224,112,96,0.12)' : 'transparent', color: filter === f ? 'var(--neon-blue)' : 'var(--muted-foreground)', border: filter === f ? '1px solid rgba(224,112,96,0.2)' : '1px solid transparent' }}>
              {f === 'needs-update' ? 'Needs Update' : f.charAt(0).toUpperCase() + f.slice(1)}
            </button>
          ))}
        </div>
        <button onClick={reload} className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', color: 'var(--muted-foreground)' }}>
      ...
      {/* States */}
      {fetchState === 'loading' && (
        <div className="flex items-center justify-center py-20 gap-3" style={glassCard}>
          <Loader size={18} className="animate-spin" style={{ color: '#E07060' }} />
          <span className="text-sm" style={{ color: 'var(--muted-foreground)' }}>Connecting to Jamf Pro…</span>
        </div>
      )}

      {(fetchState === 'no-creds' || fetchState === 'cors' || fetchState === 'auth' || fetchState === 'network' || fetchState === 'error') && (
        <div className="flex flex-col items-center justify-center py-20 gap-2 text-center" style={glassCard}>
          {fetchState === 'no-creds'
            ? <><Settings size={20} style={{ color: '#E09040' }} /><p className="text-sm mt-2" style={{ color: 'var(--foreground)' }}>No Jamf credentials</p><p className="text-xs" style={{ color: 'var(--muted-foreground)' }}>Go to Settings and add your Client ID and Secret</p></>
            : fetchState === 'auth'
            ? <><Settings size={20} style={{ color: '#E07060' }} /><p className="text-sm mt-2" style={{ color: 'var(--foreground)' }}>Invalid credentials</p><p className="text-xs" style={{ color: 'var(--muted-foreground)' }}>Check your Client ID and Secret in Settings</p></>
            : <><WifiOff size={20} style={{ color: '#E07060' }} /><p className="text-sm mt-2" style={{ color: 'var(--foreground)' }}>Cannot reach Jamf Pro</p><p className="text-xs" style={{ color: 'var(--muted-foreground)' }}>Make sure the server is running and you're on the Gap network</p></>
          }
        </div>
      )}
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} style={glassCard} className="overflow-hidden">
          <table className="w-full">
            <thead>
              <tr style={{ background: 'rgba(255,255,255,0.02)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                {['Device', 'Category', 'Location', 'IP', 'OS', 'Status', 'Updates', 'ARD'].map((col, i) => (
            ...
            <tbody>
                ...
                >
                  ...
                  <td className="py-3 px-4">
                    <span className="inline-flex items-center px-2 py-1 rounded-full text-[10px] font-semibold"
                      style={device.updateStatus === 'current' ? { background: 'rgba(224,112,96,0.1)', color: '#E07060', border: '1px solid rgba(224,112,96,0.2)' } : { background: 'rgba(224,144,64,0.1)', color: '#E09040', border: '1px solid rgba(224,144,64,0.2)' }}>
                      {device.updateStatus === 'current' ? 'Current' : 'Update'}
                    </span>
                  </td>
                  <td className="py-3 px-4 text-right">
                    {device.status === 'online' && device.ipAddress !== '—' ? (
                      <a href={`vnc://${device.ipAddress}`} className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium"
                        style={{ background: 'rgba(224,112,96,0.08)', color: 'var(--neon-blue)', border: '1px solid rgba(224,112,96,0.2)' }}>
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={8} className="py-12 text-center text-xs" style={{ color: 'var(--muted-foreground)' }}>No devices match your filter.</td></tr>
              )}
            </tbody>
          </table>
          <div className="px-4 py-3 flex items-center justify-between" style={{ borderTop: '1px solid rgba(255,255,255,0.04)' }}>
            <p className="text-[10px]" style={{ color: 'var(--muted-foreground)', opacity: 0.5 }}>
              {filtered.length} of {devices.length} devices · synced {lastSync}
            </p>
